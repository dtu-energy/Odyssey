...
class BOTorch_SingleGP_Navigator(Navigator):
    ...
    def create_acq_func(self, *args, **kwargs):
        ...
        if 'X_baseline' in params:
            params['X_baseline'] = self.mission.train_X
        if 'sampler' in params:
            params['sampler'] = SobolQMCNormalSampler(sample_shape=torch.Size([512]))
        if 'best_f' in params:
            params['best_f'] = self.mission.train_Y.max().item()
